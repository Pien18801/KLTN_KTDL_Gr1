...
filePath = "/user/root/data/products"
...
if spark._jvm.org.apache.hadoop.fs.FileSystem.get(hadoop_conf).exists(spark._jvm.org.apache.hadoop.fs.Path(filePath)):
        ...
        record_mysql = products_mysql.agg(max("id")).collect()[0][0]

        if record_mysql - record_hd > 0:
                outputDF = products_mysql.select("*").filter(products_mysql.id > record_hd)
        ...
        products_mysql.write.mode('append').parquet("hdfs://namenode:9000" + filePath)





# products_hd = spark.read.parquet("hdfs://namenode:9000/user/root/data/products")

# record_hd = products_hd.agg(max("id")).collect()[0][0]
# record_mysql = products_mysql.agg(max("id")).collect()[0][0]

# if record_mysql - record_hd > 0:
#         outputDF = products_mysql.select("*").filter(products_mysql.id > record_hd)
#         outputDF = outputDF.withColumn("created_at", lit(datetime.date(datetime.today())))
#         outputDF.write.mode('append').parquet("hdfs://namenode:9000/user/root/data/products")
#         # spark.read.parquet("hdfs://namenode:9000/user/root/data/products").createOrReplaceTempView("products")
#         # spark.sql("select count(*) from products").show()
# else:
#         spark.stop()


# products_mysql.write.mode('append').parquet("hdfs://namenode:9000/user/root/data/products")
# spark.read.parquet("hdfs://namenode:9000/user/root/data/products").createOrReplaceTempView("products")
# spark.sql("select count(*) from products").show()
# spark.stop()