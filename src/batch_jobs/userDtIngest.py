...
# userDT_hd = spark.read.parquet("hdfs://namenode:9000/user/root/data/userDetail")

# record_hd = userDT_hd.agg(max("id")).collect()[0][0]
# record_mysql = userDT_mysql.agg(max("id")).collect()[0][0]

# if record_mysql - record_hd > 0:
#         outputDF = userDT_mysql.select("*").filter(userDT_mysql.id > record_hd)
#         outputDF.write.mode('append').parquet("hdfs://namenode:9000/user/root/data/userDetail")
#         # spark.read.parquet("hdfs://namenode:9000/user/root/data/userDetail").createOrReplaceTempView("userDetail")
#         # spark.sql("select count(*) from userDetail").show()
# else:
#         spark.stop()

# userDT_mysql.write.mode('append').parquet("hdfs://namenode:9000/user/root/data/userDetail")
# spark.read.parquet("hdfs://namenode:9000/user/root/data/userDetail").createOrReplaceTempView("userDetail")
# spark.sql("select count(*) from userDetail").show()
# spark.stop()      