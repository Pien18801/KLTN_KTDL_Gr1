...
preDF = tmpProductsDF.join(tmpOrdersDF, tmpProductsDF.id == tmpOrdersDF.product_id, "inner").drop(tmpProductsDF.id)

mapDF = preDF.groupBy("product_id", "Make", "Model", "Category", "inv_quantity")\
                .agg(sum("quantity").alias("Sold"),\
                    sum("total").alias("Revenue"))

orderByProductDF = mapDF.withColumn("LeftOver", col("inv_quantity") - col("Sold")) \
        .withColumn("created_at", lit(datetime.date(datetime.today()))) \
	.select("product_id", "Make", "Model", "Category", "Sold", "Revenue", "LeftOver", "created_at") # them cot created_at
