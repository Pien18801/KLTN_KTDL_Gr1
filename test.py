...
def save_to_data_lake(batch_df, batch_id):
  batch_df.show(5)
  tableName = "testView_" + str(datetime.now().day) + "_" + str(datetime.now().month) + "_" + str(datetime.now().year)

  mapDF = batch_df.groupBy("product_id", "Make", "Model", "Category", "inv_quantity")\
                .agg(sum("quantity").alias("Sold"),\
                    sum("total").alias("Revenue"))\
                .withColumn("created_at", lit(datetime.now()))
  df = mapDF.withColumn("LeftOver", col("inv_quantity") - col("Sold"))\
            .select("product_id", "Make", "Model", "Category", "Sold", "Revenue", "LeftOver")
