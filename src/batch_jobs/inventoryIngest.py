...
inventory_mysql = spark.read.format("jdbc")\
...
filePath = "/user/root/data/inventory"
...
if spark._jvm.org.apache.hadoop.fs.FileSystem.get(hadoop_conf).exists(spark._jvm.org.apache.hadoop.fs.Path(filePath)):
        ...
        record_mysql = inventory_mysql.agg(max("id")).collect()[0][0]
        ...
        if record_mysql - record_hd > 0:
                outputDF = inventory_mysql.select("*").filter(inventory_mysql.id > record_hd)
        ...
        inventory_mysql.write.mode('append').parquet("hdfs://namenode:9000" + filePath)





# inventory_hd = spark.read.parquet("hdfs://namenode:9000/user/root/data/inventory")

# record_hd = inventory_hd.agg(max("id")).collect()[0][0]
# record_mysql = inventory_mysql.agg(max("id")).collect()[0][0]

# if record_mysql - record_hd > 0:
#         outputDF = inventory_mysql.select("*").filter(inventory_mysql.id > record_hd)
#         outputDF.write.mode('append').parquet("hdfs://namenode:9000/user/root/data/inventory")
#         # spark.read.parquet("hdfs://namenode:9000/user/root/data/inventory").createOrReplaceTempView("inventory")
#         # spark.sql("select count(*) from inventory").show()
# else:
#         spark.stop()


# inventory_mysql.write.mode('append').parquet("hdfs://namenode:9000/user/root/data/inventory")
# spark.read.parquet("hdfs://namenode:9000/user/root/data/inventory").createOrReplaceTempView("inventory")
# spark.sql("select count(*) from inventory").show()
# spark.stop()